...
AS $$
  SELECT 
    ...
    p.profile_image_url,
    p.created_at,
    COALESCE(SUM(CASE WHEN ca.created_at >= '2025-04-05' THEN a.points ELSE 0 END), 0) as total_points,
    COALESCE(SUM(CASE WHEN ca.created_at >= '2025-04-05' AND ca.date = CURRENT_DATE THEN a.points ELSE 0 END), 0) as daily_points
  FROM public.profiles p
  LEFT JOIN public.claimed_activities ca ON ca.user_id = p.id AND ca.created_at >= '2025-04-05'
